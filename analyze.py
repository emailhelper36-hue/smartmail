...
def find_id_by_text(text):
    ...
    text_clean = text.rstrip("...").strip() # Remove dots for matching
...
# ------------------------------------------------------------------
# 3. WEBHOOK (FIXED EXTRACTION LOGIC)
# ------------------------------------------------------------------
    ...
    try:
        ...
        if "message" in data:
            ...
            if isinstance(msg, dict):
                user_text = msg.get("text", "") # Extract text from dict
            else:
                user_text = str(msg) # It's already a string
        ...
        # Ensure user_text is a string and stripped
        if user_text:
